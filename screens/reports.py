...
from components.theme import action_bar, flex_columns, SUCCESS_BG, INFO_BG, WARNING_BG, DARK_PRIMARY, SECONDARY_TEXT, ERROR_BG
...
class ReportsScreen(MDScreen):
    ...
    def _build_ui(self):
        ...
        for titulo, val, col in [
            (f"Ventas día ({ventas_dia['count']} docs)", f"${ventas_dia['total']:,.0f} COP", SUCCESS_BG),
            (f"Ventas semana ({ventas_sem['count']})", f"${ventas_sem['total']:,.0f} COP", INFO_BG),
            (f"Ventas mes ({ventas_mes['count']})", f"${ventas_mes['total']:,.0f} COP", WARNING_BG),
            (f"Ventas año ({ventas_anio['count']})", f"${ventas_anio['total']:,.0f} COP", DARK_PRIMARY),
            ("Ticket promedio COP", f"${ticket:,.0f} COP", SUCCESS_BG),
            ("Productos", f"{len(repo.list_products())} SKU", SECONDARY_TEXT),
        ]:
        ...
        for titulo, val, col in [
            ("Ingresos (Pagada)", f"${estado['ingresos']:,.0f} COP", SUCCESS_BG),
            ("Costo ventas", f"${estado['costo']:,.0f} COP", ERROR_BG),
            ("Bruto", f"${estado['bruto']:,.0f} COP", INFO_BG),
            ("Impuestos IVA 19% DIAN", f"${impuestos['iva_19']:,.0f} COP", WARNING_BG),
            ("Neto", f"${estado['neto']:,.0f} COP", DARK_PRIMARY),
            ("Flujo neto", f"${flujo['neto']:,.0f} COP", SUCCESS_BG),
        ]:
            fin_grid.add_widget(MDCard(MDLabel(text=titulo, font_style="Body", role="small", halign="center", adaptive_height=True), MDLabel(text=val, font_style="Title", role="medium", halign="center", theme_text_color="Custom", text_color=col, adaptive_height=True), orientation="vertical", padding="12dp", size_hint_y=None, height="90dp", style="elevated"))
        ...
        for titulo, val, col in [
            (f"Rotación inventario {kpis['rotacion']:.2f}x", f"Días {kpis['dias_inventario']:.0f} días", WARNING_BG),
            (f"Tasa conversión {kpis['conversion']:.0f}%", f"Margen bruto {kpis['margen_bruto_pct']:.1f}%", DARK_PRIMARY),
            (f"Margen neto {kpis['margen_neto_pct']:.1f}%", f"Punto equilibrio ${kpis['punto_equilibrio']:,.0f} COP", SECONDARY_TEXT),
            ("Costos fijos mes", f"${kpis['costos_fijos']:,.0f} COP", SECONDARY_TEXT),
        ]:
